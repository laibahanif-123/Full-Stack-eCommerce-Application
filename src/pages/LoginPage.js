...
export default function LoginPage() {
  ...
  const { login, token, error, clearErrors, loading } = useStore();
  ...
  return (
    <div className="min-h-[calc(100vh-12rem)] page-container py-8 lg:py-12">
      <div className="flex flex-col lg:flex-row rounded-2xl overflow-hidden shadow-card min-h-[520px]">
        <div className="hidden lg:flex lg:w-5/12 bg-gradient-to-br from-brand-500 via-brand-600 to-orange-700 p-10 flex-col justify-between text-white relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <h1 className="text-3xl font-black mb-3">Welcome to NovaCart</h1>
            <p className="text-white/85 text-sm leading-relaxed max-w-xs">
              Pakistan&apos;s favorite online marketplace. Sign in to track orders, save wishlists, and checkout faster.
            </p>
          </div>
          <ul className="relative space-y-3 text-sm text-white/90">
            <li className="flex items-center gap-2">✓ Free shipping over Rs. 2,000</li>
            <li className="flex items-center gap-2">✓ Secure payments & easy returns</li>
            <li className="flex items-center gap-2">✓ Thousands of products daily</li>
          </ul>
        </div>
        ...
        <div className="flex-1 bg-white p-8 md:p-10 flex flex-col justify-center">
          <div className="max-w-md mx-auto w-full">
            <div className="mb-8 lg:hidden text-center">
              ...
              <p className="text-gray-500 text-sm mt-1">Login to your NovaCart account</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button type="submit" disabled={loading} className="w-full btn-primary py-3.5 rounded-xl text-sm flex items-center justify-center gap-2">
                {loading ? 'Logging in...' : 'LOGIN'} {!loading && <FiArrowRight size={16} />}
              </button>
            </form>
            ...
            <div className="mt-6 pt-6 border-t border-gray-100 text-center text-sm text-gray-500">
              New to NovaCart? <Link to="/signup" className="text-brand-500 font-semibold hover:underline">Create Account</Link>
            </div>

            <div className="text-center mt-4">
              <Link to="/admin-login" className="text-xs text-gray-400 hover:text-brand-500">Admin Login →</Link>
            </div>
          </div>
        </div>
      </div>
